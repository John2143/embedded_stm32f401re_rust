...
#[rtic::app(device = stm32f4xx_hal::pac, peripherals = false, dispatchers = [SPI3,SPI4])]
mod app {
    use defmt::println;
    ...
    //use embassy_stm32::{rcc::{low_level::RccPeripheral, Clocks}, Peripheral};
    ...
    async fn main_loop(_cx: main_loop::Context) {
        println!("main loop");
    }

    #[task(local = [input_ir_loop], priority = 2)]
    async fn ir_loop(_cx: ir_loop::Context) {
        println!("ir loop");
    }
}
